...
/*   Updated: 2023/01/13 09:38:12 by gabrielagen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_walls_around_map(t_data *data) //appeler apres check_borders sinon segfaul
{
    int x;
    int y;
...
int check_map_char(t_data *data) //appeler apres check_borders sinon segfault
{
    ...
    while (data->map[y])
    {
        ...
        while (data->map[y][x])
        {
            if (data->map[y][x] != '1' && data->map[y][x] != '0' && data->map[y][x] != 'N' && data->map[y][x] != 'S' && data->map[y][x] != 'E' && data->map[y][x] != 'W')
            {
...
int check_borders(t_data *data)
{
    ...
    {
        printf("Error: Map is empty or missing\n");
        exit(EXIT_FAILURE);
    }
    y = 0;
    while (data->map[y])

...
{
	dt->ray->ray_pos_x = dt->player_x;
	dt->ray->ray_pos_y = dt->player_y;
...
void	calcul_step_init_sidedist(t_data *dt) //calculate step and initial sideDist
{
	...
	deltadist_x = dt->ray->deltadist_x;
	deltadist_x = dt->ray->deltadist_y;
	if (dt->ray->ray_dir_x < 0)
	{
		dt->ray->step_x = -1;
		dt->ray->side_dist_x = (dt->ray->pos_x - dt->ray->map_x) * deltadist_x;
	}
	else
	{
		dt->ray->step_x = 1;
		dt->ray->side_dist_x = (dt->ray->map_x + 1.0 - dt->ray->pos_x) * deltadist_x;
	}
	if (dt->ray->ray_dir_y < 0)
	{
		dt->ray->step_y = -1;
		dt->ray->side_dist_y = (dt->ray->pos_y - dt->ray->map_y) * deltadist_y;
	}
	else
	{
		dt->ray->step_y = 1;
		dt->ray->side_dist_y = (dt->ray->map_y + 1.0 - dt->ray->pos_y) * deltadist_y;
	}
}
